    ...
    def load_model(self, model_name: str):
        ...
        if use_xformers:
            ...
            if xformers_spec is not None:
                ...
                use_xformers = False
                print("APEServer: xformers is not installed, please run `pip install xformers`")
        else:
            print(f"APEServer: Not using xformers: {use_xformers}, set OVDSERVER_USE_XFORMERS=1 to enable if xformers is installed")

        if model_name == "APE_A":
            self.load_APE_A(use_xformers=use_xformers)
        elif model_name == "APE_B":
            self.load_APE_B(use_xformers=use_xformers)
        elif model_name == "APE_C":
            self.load_APE_C(use_xformers=use_xformers)
        elif model_name == "APE_D":
