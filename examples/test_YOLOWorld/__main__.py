...
def parse_args():
    parser = argparse.ArgumentParser(description="YOLO World Test Script")
...
def test_predict_with_model_name():
    with open("assets/bus.jpg", "rb") as image_file:
        image_data = base64.b64encode(image_file.read()).decode("utf-8")
    response = requests.post(url, json={"image": image_data, "classes": ["person", "car"], "model_name": "yolov8m-world"})
    assert response.status_code == 200
    assert "result" in response.json()
    print(response.json())
